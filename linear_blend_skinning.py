...
def calculate_errors(test_model, filename=""):
    ...
    #A_orig_out = np.reshape( A_orig, (3, test_model.num_of_vertices, test_model.num_of_frames) )
    #for f in range(0, test_model.num_of_frames):
    #    A_orig_out_final = np.transpose(A_orig_out[:, :, f])
    #    print("VASILIS A_orig_out shape: ", A_orig_out.shape)
    #    np.savetxt(os.path.dirname(__file__)+'\\Original\\'+filename+"_"+str(f)+"_TESTMODEL_ORIGINAL.obj", A_orig_out_final, fmt="%.6f", delimiter=' ')

    np.savetxt(os.path.dirname(__file__)+'\\'+filename+"TESTMODEL_ORIGINAL.txt", A_orig, fmt="%.6f", delimiter=' ')
    np.savetxt(os.path.dirname(__file__)+'\\'+filename+"TESTMODEL_APPROXIMATED.txt", A_appr, fmt="%.6f", delimiter=' ')
