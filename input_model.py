...
class TrainModel(Model):
	...
	def get_train_labels(self):
		return self.labels_for_rest_pose
		...
		for i in range(0, self.num_of_vertices):
			for j in range(0, self.num_of_bones ):
				if i in self.v_groups[j]:
					if (filepath == None):
						labels[i][j] = self.v_groups_weights[j][self.v_groups[j].index(i)] 
					else:
						labels[i][int(newIndices[j])] = self.v_groups_weights[j][self.v_groups[j].index(i)] 
