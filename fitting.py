...
def __construct_A_matrix_for_w(test_model, v, initial_v_weights_indices):
    weights_size = 6
    A = np.zeros( (3*(test_model.num_of_frames+1), weights_size) )
    i = -1
    #print("----------- VASILIS A ---------- \n", A.shape, "\n", A[0].shape)
    #print("----------- VASILIS v ---------- \n", v)
    #print("----------- VASILIS initial_v_weights_indices ---------- \n", len(initial_v_weights_indices), "\n", initial_v_weights_indices[0])
    #print("----------- VASILIS test_model.affine_transformations ----------- \n", len(test_model.affine_transformations))

    for f in range(0, test_model.num_of_frames+1):
        if f <= test_model.num_of_frames-1:
            i = 0
            for bone in test_model.v_weights_indices[v]:
                if weights_size > i and bone in initial_v_weights_indices[v]:
