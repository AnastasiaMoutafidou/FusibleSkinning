...
def start_deepSkinning_rignet(paths_for_Blender, test_model_name, networkPrefix):

    # ------------ RIGNET OUTPUT --------------
    print("\n------------------- RIGNET MODEL (USING TRAINMODEL TO GET RIGNET MODEL ACTUAL WEIGHTS): ---------------------\n")
    bpy.ops.import_scene.fbx(filepath = paths_for_Blender.rignet_model_path)
    rignet_model_names_list = get_model_names()
    rignet_model = input_model.TrainModel(rignet_model_names_list[0], rignet_model_names_list[1], rignet_model_names_list[2])
    rignet_model.change_mesh_to_World_coordinates()
    rignet_model.number_of_proxy_bones = number_of_proxy_bones
    rignet_model.set_v_groups_names()
    rignet_model.set_v_groups()
    rignet_model.set_mesh_v_in_all_frames()
    for action in bpy.data.actions:
        bpy.data.actions.remove(action, do_unlink=True, do_id_user=True, do_ui_user=True)
    rignet_model.set_train_labels()
    rignet_labels = rignet_model.get_train_labels() # GET ACTUAL WEIGHTS IN REST POSE FOR RIGNET
    # ----------------------------------------
    ...
    #NETWORK LOADING
    model = keras.models.load_model(os.path.dirname(__file__)+'\\pretrainedNETs\\'+networkPrefix+'.h5')
    model.summary()
    batch_size = 4096
    ...
    if paths_for_Blender.file_type != 'fbx':
        ...
        np.savetxt(os.path.dirname(__file__)+'\\outcome\\'+networkPrefix+'RignetInputSequence.txt', starting_sequence, fmt="%.6f", delimiter=' ')
    ...
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'DEEPSKINNING-'+test_model_name+'CLUSTERS.txt', clusters_for_pCenter_list, fmt="%.2f")



    # the vertices that were predicted to belong to zero bones
    # are then assigned to the one with the highest probability
    test_set_labels = assign_unassigned_vertices_to_bone(test_set_labels, rignet_labels)
    test_model.set_labels(test_set_labels)
    v_weightsDS, v_weights_indicesDS = weight_calculation.calculate_weights_using_tensorflow_predictions(test_model, predictions)

    print("------------- PREDICTIONS (AFTER) --------------\n", predictions[0])
    print("------------- TEST LABELS (AFTER) --------------\n\nDEEP SKINNING END\n", test_set_labels[0])

    #----------------------------- DEEP SKINNING WEIGHTS ----------------------------- (END)

    #----------------------------- RIGNET WEIGHTS -----------------------------
    # RIGNET - COPY THE ACTUAL WEIGHTS FROM RIGNET OUTPUT
    test_set_labels = np.copy(rignet_labels)
    ...
    print("\n------------- CLUSTERS FOR P-CENTER --------------\n", clusters_for_pCenter)
    clusters_for_pCenter_list = [clusters_for_pCenter]
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'RIGNET-'+test_model_name+'CLUSTERS.txt', clusters_for_pCenter_list, fmt="%.2f")
    ...
    start = timer()
    v_weights, v_weights_indices = weight_calculation.calculate_weights_using_tensorflow_predictions(test_model, rignet_labels)
    v_weights_final = np.copy(v_weights)
    print("v_weights shape", len(v_weights), len(v_weights[0]))
    for v in range(0, len(v_weights_final) ):
        for i in range (0, len(v_weights_final[v])):
            min_distance = 1.7976931348623157e+308
            min_index = -1
            for j in range(0, len(v_weightsDS[v]) ):
                distance = abs(v_weights[v][i] - v_weightsDS[v][j])
                if (min_distance > distance):
                    min_index = j
                    min_distance = distance
            v_weights_final[v][i] = v_weights[v][i]*0.5 + v_weightsDS[v][min_index]*0.5

    test_set_labels = np.copy(rignet_labels)

    for w in range(0, len(v_weights_final) ):
        sumRignetDS = np.sum(v_weights_final[w])
        if (w == 0):
            print("np.sum(v_weights_final["+ str(w) +"]): ", sumRignetDS)
        for i in range (0, len(v_weights_final[w])):                
            #print("(BEFORE) v_weights_final["+str(w)+"]["+str(i)+"]: ", v_weights_final[w][i])
            v_weights_final[w][i] /= sumRignetDS
            #print("(AFTER) v_weights_final["+str(w)+"]["+str(i)+"]: ", v_weights_final[w][i])
            #test_set_labels[w][v_weights_indices[w][i]] = v_weights_final[w][i]

    # SET LABELS ONCE AGAIN
    number_of_max_probs = 6
    indices_test_set_labels = np.zeros((test_set_labels.shape[0], number_of_max_probs), dtype=np.int)
    for i in range(0, test_set_labels.shape[0]):
        indices_test_set_labels[i] = np.argsort(-test_set_labels[i])[:number_of_max_probs]
    ...
    print("\nRIGNET DEEP SKINNING TOGETHER------------- TEST LABELS --------------\n", test_set_labels[0])
    print("------------- INDICES MAX LABELS --------------\n", indices_test_set_labels[0])
    print("------------- RIGNET WEIGHTS (SO BEFORE STATE) --------------\n", rignet_labels[0])
    ...
    print("\n------------- CLUSTERS FOR P-CENTER --------------\n", clusters_for_pCenter)
    clusters_for_pCenter_list = [clusters_for_pCenter]
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'RIGNETDEEPSKINNING-'+test_model_name+'CLUSTERS.txt', clusters_for_pCenter_list, fmt="%.2f")
    ...
    test_model.set_labels(test_set_labels)
    print("------------- PREDICTIONS (AFTER) --------------\n", rignet_labels[0])
    print("------------- TEST LABELS (AFTER) --------------\n\nRIGNET DEEP SKINNING END", test_set_labels[0])
    #----------------------------- RIGNET WEIGHTS ----------------------------- (END)


    test_model.set_v_weights(v_weights_final, v_weights_indices)

    print("------------- PREDICTIONS - WEIGHTS (AFTER) DEEP SKINNING--------------\n", v_weightsDS[0])
    print("------------- PREDICTIONS - WEIGHTS (AFTER) SUM DEEP SKINNING:  --------------\n", np.sum(v_weightsDS[0]))
    print("------------- PREDICTIONS - WEIGHTS (AFTER) RIGNET --------------\n", v_weights[0])
    print("------------- PREDICTIONS - WEIGHTS (AFTER) SUM RIGNET:  --------------\n", np.sum(v_weights[0]))
    print("------------- PREDICTIONS - WEIGHTS TEST MODEL (AFTER) RIGNET DEEP SKINNING --------------\n", test_model.v_weights[0])
    print("------------- PREDICTIONS - WEIGHTS TEST MODEL (AFTER) SUM RIGNET DEEP SKINNING --------------\n", np.sum(test_model.v_weights[0]))

    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'DEEPSKINNINGweightsg.csv', v_weightsDS, fmt="%.6f", delimiter='|')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'DEEPSKINNINGweights-indices.csv', v_weights_indicesDS, fmt="%.6f", delimiter='|')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'RIGNETweights.csv', v_weights, fmt="%.6f", delimiter='|')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'RIGNETweights-indices.csv', v_weights_indices, fmt="%.6f", delimiter='|')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'RIGNETDEEPSKINNINGweights.csv', v_weights_final, fmt="%.6f", delimiter='|')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'RIGNETDEEPSKINNINGweights-indices.csv', v_weights_indices, fmt="%.6f", delimiter='|')
    ...
    data = bpy.data.objects[test_model.name_of_obj_mesh].data
    #data = bpy.data.objects['tree_forg'].data
    #data = bpy.data.meshes['-gallop-01']
    dataFaces = np.zeros( (len(data.polygons), 3) )
    print("len(data.polygons):" + str(len(data.polygons)))
    a = 0
    j = 0
    for poly in data.polygons:
        ...
        for loop_index in range(poly.loop_start, poly.loop_start + poly.loop_total):
            ...
            dataFaces[a,j] = data.loops[loop_index].vertex_index+1
            j += 1
        a+=1
            #print("    UV: %r" % uv_layer[loop_index].uv)
       # for idx in f.vertices:
    #print("Polygons:" + dataFaces)
    ...
    print("len(data.polygons): ", len(data.polygons))
    dataNormals_orig = np.zeros( (3*len(data.polygons), test_model.num_of_frames) )
    dataNormals_approx = np.zeros( (3*len(data.polygons), test_model.num_of_frames) )
    for f in range(0, test_model.num_of_frames):
        for face in range(0, len(dataFaces)):
            faceVertices_orig = np.zeros((3, 3))
            faceVertices_approx = np.zeros((3, 3))
            for vertice in range(0, len(dataFaces[0])):
                v = int(dataFaces[face, vertice] - 1)
                #print("v: ", v)
            ...
            dataNormals_orig[(face*3), f] = normal_orig[0]
            dataNormals_orig[(face*3)+1, f] = normal_orig[1]
            dataNormals_orig[(face*3)+2, f] = normal_orig[2]
            normal_approx = np.asarray(Normal(faceVertices_approx[0], faceVertices_approx[1], faceVertices_approx[2]))
            dataNormals_approx[(face*3), f] = normal_approx[0]
            dataNormals_approx[(face*3)+1, f] = normal_approx[1]
            dataNormals_approx[(face*3)+2, f] = normal_approx[2]

    # print("------------- ORIGINAL ----------------\n", len(dataNormals_orig), ", ", len(dataNormals_orig[0]))
    # print("------------- APPROXIMATED ----------------\n", len(dataNormals_approx), ", ", len(dataNormals_approx[0]))
    # print("------------- ORIGINAL ----------------\n", dataNormals_orig[0,0], ", ", dataNormals_orig[1,0], ", ", dataNormals_orig[2,0])
    # print("------------- APPROXIMATED ----------------\n", dataNormals_approx[0,0], ", ", dataNormals_approx[1,0], ", ", dataNormals_approx[2,0])
    dataNormals_orig = np.reshape(dataNormals_orig, (len(data.polygons), 3, -1))
    dataNormals_approx = np.reshape(dataNormals_approx, (len(data.polygons), 3, -1))

    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+"RIGNET"+test_model_name+'Normals-Original.txt', dataNormals_orig[:,:,0], fmt="%.6f", delimiter=' ')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+"RIGNET"+test_model_name+'Normals-Approximation.txt', dataNormals_approx[:,:,0], fmt="%.6f", delimiter=' ')

    Normals_Distance  = np.zeros( (dataNormals_orig.shape[0], test_model.num_of_frames) )
    # AVERAGE MAX DISTANCE ERROR METRIC
    for f in range(0, dataNormals_orig.shape[1]):
        a = 0
        for v in range(0, dataNormals_orig.shape[0]):
            normal_xyz_orig = np.array([ dataNormals_orig[v, 0, f], dataNormals_orig[v, 1, f], dataNormals_orig[v, 2, f] ])
            normal_xyz_approx = np.array([dataNormals_approx[v, 0, f], dataNormals_approx[v, 1, f], dataNormals_approx[v, 2, f] ])
    ...
    for i in range(0, number_of_fitting_steps):
        ...
        Normals_Distance  = np.zeros( (dataNormals_orig.shape[0], test_model.num_of_frames) )
        # AVERAGE MAX DISTANCE ERROR METRIC
        for f in range(0, dataNormals_orig.shape[2]):
            a = 0
            for v in range(0, dataNormals_orig.shape[0]):
                normal_xyz_orig = np.array([ dataNormals_orig[v, 0, f], dataNormals_orig[v, 1, f], dataNormals_orig[v, 2, f] ])
                normal_xyz_approx = np.array([dataNormals_approx[v, 0, f], dataNormals_approx[v, 1, f], dataNormals_approx[v, 2, f] ])
        ...
        if (i == (number_of_fitting_steps - 1)):
            np.savetxt(os.path.dirname(__file__)+'\\outcome\\'+networkPrefix+'RIGNET-DEEP_SKINNING-FinalTransformations.txt', np.transpose(new_transformations), fmt="%.6f", delimiter=' ')
            np.savetxt(os.path.dirname(__file__)+'\\outcome\\'+networkPrefix+'RIGNET-DEEP_SKINNING-FinalWeights.txt', new_v_weights, fmt="%.6f", delimiter=' ')
            np.savetxt(os.path.dirname(__file__)+'\\outcome\\'+networkPrefix+'RIGNET-DEEP_SKINNING-FinalWeightIndices.txt', new_v_weights_indices, fmt="%.df", delimiter=' ')
            print("Approximated v: ", len(approx_v))
            for f in range(0, test_model.num_of_frames):
                print("frame: ", str(f))
                np.savetxt(os.path.dirname(__file__)+'\\outcome\\'+networkPrefix+"RIGNET-DEEP_SKINNING--"+str(f)+'FinalApproximatedSequence.txt', approx_v[f], fmt="%.6f", delimiter=' ')
            approximated_sequence = np.reshape(approx_v, (-1, 3))
            np.savetxt(os.path.dirname(__file__)+'\\outcome\\'+networkPrefix+"RIGNET-DEEP_SKINNING-"+'FinalApproximatedSequence.txt', approximated_sequence, fmt="%.6f", delimiter=' ')
    ...
    if index_of_min_error % 2 == 1:
        ...
        # np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'RIGNETFinalTransformations.csv', np.transpose(old_w_t[index_of_min_error]), fmt="%.6f", delimiter=' ')
        # np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'RIGNETFinalWeights.csv', old_w_t[index_of_min_error - 1], fmt="%.6f", delimiter=' ')
    else:
    ...
    #     np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'RIGNETFinalTransformations.csv', np.transpose(old_w_t[index_of_min_error - 1]), fmt="%.6f", delimiter=' ')
    #     np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'RIGNETFinalWeights.csv', old_w_t[index_of_min_error], fmt="%.6f", delimiter=' ')
    # np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'RIGNETFinalWeightIndices.csv', old_v_weights_indices[index_of_min_error // 2], fmt="%.df", delimiter=' ')
    ...
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+"RIGNET-DEEP_SKINNING"+test_model_name+'Dis_Per.txt', np.asarray(DistortionPercentage), fmt="%.6f", delimiter='|')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+"RIGNET-DEEP_SKINNING"+test_model_name+'RMS_E.txt', np.asarray(RMS_error), fmt="%.6f", delimiter='|')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+"RIGNET-DEEP_SKINNING"+test_model_name+'Avg_Max_Dis.txt', np.asarray(AVG_MAX_Distance), fmt="%.6f", delimiter='|')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+"RIGNET-DEEP_SKINNING"+test_model_name+'Avg_Normal_Angle_Dist.txt', np.asarray(AVG_NORMAL_ANGLE_DISTANCE), fmt="%.6f", delimiter='|')

    data = bpy.data.objects['Spiderman'].data
    # data = bpy.data.objects['tree_forg'].data
    # data = bpy.data.meshes['direction-01']
    dataFaces = np.zeros( (len(data.polygons), 3) )
    print("len(data.polygons):" + str(len(data.polygons)))
    i = 0
    j = 0
    for poly in data.polygons:
        ...
        for loop_index in range(poly.loop_start, poly.loop_start + poly.loop_total):
            #print("Vertex: %d" % data.loops[loop_index].vertex_index)
            dataFaces[i,j] = data.loops[loop_index].vertex_index+1
            j += 1
        i+=1
            #print("    UV: %r" % uv_layer[loop_index].uv)
       # for idx in f.vertices:
    ...
    for f in range(0, test_model.num_of_frames):
        np.savetxt(os.path.dirname(__file__)+'\\DSRignet\\'+networkPrefix +"RIGNET"+test_model_name+"_color"+str(f)+".obj", ApproximatedWithColor[:,:,f], fmt="%.6f", newline="\nv ",header="# model approximated!")
        with open(os.path.dirname(__file__)+'\\DSRignet\\'+networkPrefix +"RIGNET"+test_model_name+"_color"+str(f)+".obj", "ab") as file:
            np.savetxt(file, dataFaces,fmt="%d", newline="\nf ")  
   


    np.savetxt(os.path.dirname(__file__)+'\\DSRignet\\'+networkPrefix +"RIGNET-"+"change"+"rest_pose.txt", test_model_mesh_rest_pose, fmt="%.6f", newline="\nv ")
    for f in range(0, test_model.num_of_frames):
        np.savetxt(os.path.dirname(__file__)+'\\DSRignet\\'+networkPrefix +"RIGNET-"+"change"+"_approx"+str(f)+".obj", Approximated[:,:,f], fmt="%.6f", newline="\nv ",header="# gallop model approximated!")
        with open(os.path.dirname(__file__)+'\\DSRignet\\'+networkPrefix +"RIGNET-"+"change"+"_approx"+str(f)+".obj", "ab") as file:
            np.savetxt(file, dataFaces,fmt="%d", newline="\nf ")
    ...
    #clusters_path = os.path.dirname(__file__)+'\\spiderman\\clusters\\'
    #os.mkdir(clusters_path, mode=0o777);
    for f in range(0, test_model.num_of_frames):
        np.savetxt(os.path.dirname(__file__)+'\\DSRignet\\'+networkPrefix +"RIGNET"+test_model_name+"_approx"+str(f)+".obj", ApproximatedWithColor[:,:,f], fmt="%.6f", newline="\nv ",header="# gallop model approximated!")
        with open(os.path.dirname(__file__)+'\\DSRignet\\'+networkPrefix +"RIGNET"+test_model_name+"_approx"+str(f)+".obj", "ab") as file:
            np.savetxt(file, dataFaces,fmt="%d", newline="\nf ")
