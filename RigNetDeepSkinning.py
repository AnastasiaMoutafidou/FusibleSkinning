...
def start_rignet_deepSkinning(paths_for_Blender, test_model_name, networkPrefix):
    ...
    for v in range(0, len(v_weights_final) ):
        for i in range (0, len(v_weights_final[v])):
            ...
            min_index = -1
            for j in range(0, len(v_weights[v]) ):
                distance = abs(v_weightsDS[v][i] - v_weights[v][j])
                if (min_distance > distance):
                    min_index = j
                    min_distance = distance
            v_weights_final[v][i] = v_weightsDS[v][i]*0.5 + v_weights[v][min_index]*0.5
    ...
    print("\nDEEP SKINNING RIGNET TOGETHER------------- TEST LABELS --------------\n", test_set_labels[0])
    print("------------- INDICES MAX LABELS --------------\n", indices_test_set_labels[0])
    ...
    test_model.set_labels(test_set_labels)
    print("------------- PREDICTIONS (AFTER) --------------\n", rignet_labels[0])
    print("------------- TEST LABELS (AFTER) --------------\n\nDEEP SKINNING RIGNET END", test_set_labels[0])
    #----------------------------- RIGNET WEIGHTS ----------------------------- (END)
    ...
    print("------------- PREDICTIONS - WEIGHTS TEST MODEL (AFTER) DEEP SKINNING RIGNET --------------\n", test_model.v_weights[0])
    print("------------- PREDICTIONS - WEIGHTS TEST MODEL (AFTER) SUM DEEP SKINNING RIGNET --------------\n", np.sum(test_model.v_weights[0]))
    ...
    for f in range(0, test_model.num_of_frames):
        np.savetxt(os.path.dirname(__file__)+'\\RIGNETDS\\'+networkPrefix +"RIGNETDS"+test_model_name+"_color"+str(f)+".obj", ApproximatedWithColor[:,:,f], fmt="%.6f", newline="\nv ",header="# model approximated!")
        with open(os.path.dirname(__file__)+'\\RIGNETDS\\'+networkPrefix +"RIGNETDS"+test_model_name+"_color"+str(f)+".obj", "ab") as file:
            np.savetxt(file, dataFaces,fmt="%d", newline="\nf ")  
   


    np.savetxt(os.path.dirname(__file__)+'\\RIGNETDS\\'+networkPrefix +"RIGNETDS-"+"rest_pose.txt", test_model_mesh_rest_pose, fmt="%.6f", newline="\nv ")
    for f in range(0, test_model.num_of_frames):
        np.savetxt(os.path.dirname(__file__)+'\\RIGNETDS\\'+networkPrefix +"RIGNETDS-"+"_approx"+str(f)+".obj", Approximated[:,:,f], fmt="%.6f", newline="\nv ",header="# model approximated!")
        with open(os.path.dirname(__file__)+'\\RIGNETDS\\'+networkPrefix +"RIGNETDS-"+"_approx"+str(f)+".obj", "ab") as file:
            np.savetxt(file, dataFaces,fmt="%d", newline="\nf ")
    ...
    for f in range(0, test_model.num_of_frames):
        np.savetxt(os.path.dirname(__file__)+'\\lala\\'+networkPrefix +"RIGNETDS"+test_model_name+"_approx"+str(f)+".obj", ApproximatedWithColor[:,:,f], fmt="%.6f", newline="\nv ",header="# gallop model approximated!")
        with open(os.path.dirname(__file__)+'\\lala\\'+networkPrefix +"RIGNETDS"+test_model_name+"_approx"+str(f)+".obj", "ab") as file:
            np.savetxt(file, dataFaces,fmt="%d", newline="\nf ")
