...
# calculate normal from 4 verts found in the blender sources in arithb.x
...
def start_tensorflow_without_training(paths_for_Blender, networkPrefix, test_model_name):
    ...
    np.savetxt(os.path.dirname(__file__)+'\\Original\\'+test_model_name+'InputSequence.txt', starting_sequence, fmt="%.6f", delimiter=' ')
    ...
    print("------------- WEIGHTS SHAPE -------------\n", len(v_weights), " - ", len(v_weights[0]))
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'weights.txt', v_weights, fmt="%s", delimiter=' ')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'weights-indices.txt', v_weights_indices, fmt="%s", delimiter=' ')
    ...
    facesSize = data.polygons[0].loop_start + data.polygons[0].loop_total
    dataFaces = np.zeros( (len(data.polygons), facesSize) )
    ...
    for f in range(0, test_model.num_of_frames):
        for face in range(0, len(dataFaces)):
            faceVertices_orig = np.zeros((facesSize, 3))
            faceVertices_approx = np.zeros((facesSize, 3))
    ...
    #
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+test_model_name+'Normals-Original.txt', dataNormals_orig[:,:,0], fmt="%.6f", delimiter=' ')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+test_model_name+'Normals-Approximation.txt', dataNormals_approx[:,:,0], fmt="%.6f", delimiter=' ')
    #
    ...
    #
    initial_dE, initial_E_RMS, initial_Avg_Max_Distance_Error = linear_blend_skinning.calculate_errors(test_model, test_model_name)
    ...
    for i in range(0, number_of_fitting_steps):
        ...
        start = timer()
        new_dE, new_E_RMS, new_Avg_Max_Distance_Error = linear_blend_skinning.calculate_errors(test_model, test_model_name)
    ...
    start = timer()
    dE, E_RMS, Avg_Max_Distance_Error = linear_blend_skinning.calculate_errors(test_model, test_model_name)
    end = timer()
    time_for_each_step.append('\tto calculate errors : ' + str(end - start))

    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'weights.txt', test_model.v_weights, fmt="%s", delimiter=' ')
    np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+'weights-indices.txt', test_model.v_weights_indices, fmt="%s", delimiter=' ')
    ...
    for f in range(0, test_model.num_of_frames):
        np.savetxt(os.path.dirname(__file__)+'\\DeepSkinningColors\\'+networkPrefix +test_model_name+"_color"+str(f)+".obj", ApproximatedWithColor[:,:,f], fmt="%.6f", newline="\nv ",header="# model approximated!")
        with open(os.path.dirname(__file__)+'\\DeepSkinningColors\\'+networkPrefix +test_model_name+"_color"+str(f)+".obj", "ab") as file:
            np.savetxt(file, dataFaces,fmt="%d", newline="\nf ")  
    

    np.savetxt(os.path.dirname(__file__)+'\\DS\\'+networkPrefix+"-"+"DS"+"rest_pose.txt", test_model_mesh_rest_pose, fmt="%.6f", newline="\nv ")
    for f in range(0, test_model.num_of_frames):
        np.savetxt(os.path.dirname(__file__)+'\\DS\\'+networkPrefix+"-"+"DS"+"_approx"+str(f)+".obj", Approximated[:,:,f], fmt="%.6f", newline="\nv ",header="# gallop model approximated!")
        with open(os.path.dirname(__file__)+'\\DS\\'+networkPrefix+"-"+"DS"+"_approx"+str(f)+".obj", "ab") as file:
            np.savetxt(file, dataFaces,fmt="%d", newline="\nf ")
    ...
    if ( facesSize == 3):
        ...
        for f in range(0, test_model.num_of_frames):
            np.savetxt(os.path.dirname(__file__)+'\\DeepSkinningColors\\'+networkPrefix+test_model_name+"_approx"+str(f)+".obj", ApproximatedWithColor[:,:,f], fmt="%.6f", newline="\nv ",header="#model approximated!")
            with open(os.path.dirname(__file__)+'\\DeepSkinningColors\\'+networkPrefix+test_model_name+"_approx"+str(f)+".obj", "ab") as file:
                np.savetxt(file, dataFaces,fmt="%d", newline="\nf ")
